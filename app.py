...
def check_price_alerts():
    ...
    for item in items_to_alert:
        alert_message = f"Price Drop Alert! '{item.product_name[:50]}...' is now ₹{item.current_price} (your target was ₹{item.target_price})."
...
def set_target_price(item_id):
    ...
    try:
        ...
        if target_price:
            ...
            db.session.commit()
            flash(f"Alert set for '{item.product_name[:30]}...' below ₹{target_price}", 'success')
        else:
...
def get_price_history_chart(item_id):
    ...
    try:
        ...
        min_price = min(prices)
        plt.axhline(y=min_price, color='green', linestyle='--', label=f'Lowest: ₹{min_price}')
        plt.legend()
...
def search():
    try:
        ...
        if not product_name: return jsonify({'error': 'Please enter a product name'})
            
        print(f"🔍 Searching for: {product_name} (User: {current_user.email})")
        ...
        if products:
            ...
            with app.app_context():
                ...
                for product in final_products:
                    if not product.get('url'): continue
                    ...
                    try:
                        ...
                        if item:
                            if current_price is not None and item.current_price != current_price:
...
def process_chat_message(message):
    ...
    try:
        # Auto-select a supported model from the API and request a response
        def _choose_model_from_list():
            try:
                models = genai.list_models()
                names = []
                if isinstance(models, dict):
                    candidates = models.get('models') or models.get('model') or list(models.values())
                else:
                    candidates = models
        ...
        chosen_model = _choose_model_from_list() if genai is not None else None
        if chosen_model:
            print(f"Using AI model: {chosen_model}")
            model = genai.GenerativeModel(chosen_model)
        else:
            # Final fallback to a broadly available model name
        ...
        # If the model name is not available for the configured API/version, attempt to list models
        print(f"AI Error: {e}")
        try:
            if genai is not None:
                ...
                if candidates:
                    ...
                    print("Available AI models:", model_names)
                    available_str = ', '.join(model_names[:10])
                    return {
                        "text": f"Requested AI model not available. Available models: {available_str}. I can still help you search!",
                        "action": None
                    }
        except Exception as e2:
            print(f"List models failed: {e2}")

        return {
            "text": "I'm having trouble connecting to the AI brain right now. But I can still help you search!",
            "action": None
        }
